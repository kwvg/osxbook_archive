...
typeToString (uint32_t    Type)
{
    switch (Type)
        ...
        case FSE_FINDER_INFO_CHANGED:
            return ("Finder Info   "); /* 6 */
        case FSE_XATTR_MODIFIED:
            return      ("Changed xattr "); /* 9 */
        case FSE_XATTR_REMOVED:
            return       ("Removed xattr "); /* 10 */
        case FSE_DOCID_CREATED:
            return ("DocID created "); // 11
        case FSE_DOCID_CHANGED:
            return ("DocID changed "); // 12
        default :
            return ("?! ");
        }
}
...
doArg(char *arg, int Print)
{
    ...
    switch (*argType)
        ...
        case FSE_ARG_DEV:
            dev = (dev_t *) argVal;
            if (g_dumpArgs) printf ("DEV: %d,%d ", major(*dev), minor(*dev));
            break;
        case FSE_ARG_MODE:
            if (g_dumpArgs) printf("MODE: %x ", *argVal);
            break;
        case FSE_ARG_PATH:
            printf ("PATH: " );
            break;
        case FSE_ARG_INO:
            if (g_dumpArgs) printf ("INODE: %d ", *argVal);
            break;
        case FSE_ARG_UID:
            if (g_dumpArgs) printf ("UID: %d ", *argVal);
            break;
        case FSE_ARG_GID:
            if (g_dumpArgs) printf ("GID: %d ", *argVal);
            break;
                            ...
                            if (fse->type == FSE_CREATE_FILE && autolink && (fse->pid != getpid()))
                                {
                                    ...
                                    if (rc)
                                        {
                                            fprintf(stderr,"%sWarning: Unable to autolink %s%s - file must have been deleted already\n",
                                                    color ? RED : "",
                                                    fse_arg->data,
                                                    color ? NORMAL : "");
                                        }
                                        ...
                                        {
                                            fprintf(stderr,"%sAuto-linked %s to %s%s\n",
                                                    color ? GREEN : "",
