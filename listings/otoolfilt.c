...
void getJToolLazyBindData (char *File)
{
    ...
    if (!newline)
        {
            fprintf(stderr,"Unable to get lazy_bind data from jtool?! No first newline\n");
            return ;
        }
    newline = strchr(newline +1, '\n') ;
    if (!newline)
        {
            fprintf(stderr,"Unable to get lazy_bind data from jtool?! No second newline\n");
            return ;
        }
    // Ok. Now let's talk.
    int l = 0;
    ...
    while ((newline = strchr(newline + 1, '\n')) != NULL)
        {
            ...
            *newline ='\0';
            ...
            if (getenv ("JDEBUG"))
                {
                    fprintf(stderr,"Added lazy bind: %s\n",
...
void getJToolStubData (char *File)
{
    ...
    if (!stubs)
        {
            fprintf(stderr,"Warning - can't get stubs. Wait till J puts a better mechanism in place..\n");
            return; // not fatal
        }
    // seek back "Mem: "
    int i = (stubs - data);
    ...
    if (argc > 1)
        {
            ...
            if (rc < 0)
                {
                    fprintf(stderr,"I can't find and/or execute otool(1). Is xcode installed?\n");
                    exit(1);
                }
