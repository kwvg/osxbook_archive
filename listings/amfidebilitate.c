...
//    NSLog(CFSTR("Processing file %s\n"), Path);
    ...
    if (mh->magic != MH_MAGIC_64)
        {
            ...
            if (mh->magic == FAT_CIGAM)
                ...
                {
                    error ("Found magic 0x%x at off file - this is not an MH_MAGIC_64 nor a FAT_MAGIC..\n", mh->magic);
                    return(NULL);
                }
        }
    ...
    else if (memmem(b,ntohl(b->size), "Apple Certi", 10))
        {
            free(fileContents);
    ...
    if (!match)
        {
            fprintf(stderr,"Can't find a CD Blob match\n");
            return (NULL);
        }
    ...
    while (amfidPid  <1 )
        {
            if (amfidPid = -1)
                {
                    debug("Error getting PID from task port. Was I handed an invalid task port?\n");
                }
            status("HERE - 0x%x, %d\n", AmfidPort, amfidPid);
            printf("Sleeping\n");
            fflush(NULL);
...
retry:
    ...
    if (kr ==0 )
        {
            status("patched AMFI through port 0x%x  @0x%llx to Faulting addr: 0x%llx\n",AmfidPort,amfidTEXTaddress + MVSACI_offset, faultingAddr );
        }
    else
        {
            printf("KR: %d\n", kr);
...
int getKqueueForPid (pid_t pid)
{
    ...
    if (kq == -1)
        {
            perror("kqueue");
            printf("UNABLE TO CREATE KQUEUE\n");
            return -1;
        }
...
#define CS_OPS_ENTITLEMENTS_BLOB 7
    ...
    struct blob *entBlob = alloca(1024);
    int entBlobLen = 1024;
    ...
    if (tsp == MACH_PORT_NULL)
        {
            ...
            if (kr != KERN_SUCCESS)
                {
                    error("Can't get amfid's task port\n");
                    exit(12);
                }
            else
                {
                    status("GOT AMFID (PID %d)'s  PORT %d\n", amfidPid,  g_AmfidPort);
                }
        }
