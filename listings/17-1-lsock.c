...
// sprintf(timeBuf, "%d......", num);
    ...
    if (nud->pname[0])
        {
            sprintf(returned + strlen(returned), "(%s)",   nud->pname );
        }
    else
        {
...
// sprintf(returned+strlen(returned), "%d\t", ntd->sndbufused);
    ...
    if (ntd->pname[0])
        {
            sprintf(returned + strlen(returned), "(%s)",   ntd->pname );
        }
    else
        {
            sprintf(returned + strlen(returned), "                 ");
                    ...
                    if (ch != ERR)
                        {
                            ...
                            if (ch == 'U' || ch =='u')
                                {
                                    wantUDP = !wantUDP;
                                    setMessage("Toggling UDP sockets");
                                }
                            if (ch == 'T' || ch == 't')
                                {
                                    wantTCP = !wantTCP;
                                    setMessage("Toggling TCP sockets");
                                }
                            if (ch == 'L' || ch == 'l')
                                {
